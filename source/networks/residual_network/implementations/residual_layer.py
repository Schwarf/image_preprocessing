...
class ResidualLayer(IResidualLayer, tensorflow.keras.layers.Layer, object):
    ...
    def __init__(self, builder: IResidualLayerParameters):
        super(ResidualLayer, self).__init__(name=builder.name)
    ...
    def _construct_residual_layer(self):
        ...
        if self._stride_size != 1:
            self._down_sample_node = tensorflow.keras.Sequential()
            self._down_sample_node.add(self._down_sample_convolution)
            self._down_sample_node.add(self._down_sample_batch_normalization)
        else:
            self._down_sample_node = lambda identity: identity

    def call(self, input_data, training=None, **kwargs):
        down_sample_node = self._down_sample_node(input_data)

        compute_convolution_1 = self._convolution_1(input_data)
        compute_batch_normalization_1 = self._batch_normalization_1(compute_convolution_1, training=training)
        ...
        down_sample = tensorflow.keras.layers.add([down_sample_node, compute_batch_normalization_2])
